...
const source = await readFile(resolve(process.cwd(), "contracts/TinyBounty.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "TinyBounty.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["TinyBounty.sol"]?.TinyBounty;
if (!contract) {
...
console.log(`Deploying TinyBounty from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_TINY_BOUNTY_CONTRACT_ADDRESS to this contract address in Vercel.");
